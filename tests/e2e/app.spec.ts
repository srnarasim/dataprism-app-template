...
test.describe('DataPrism App Template', () => {
  ...
  test('displays main sections', async ({ page }) => {
    ...
    await expect(page.getByText('Upload a file and configure chart settings to get started')).toBeVisible();
  });

  test('has working footer links', async ({ page }) => {
    const dataPrismLink = page.getByRole('link', { name: /DataPrism/i });
    await expect(dataPrismLink).toHaveAttribute('href', 'https://github.com/srnarasim/dataprism-core');
    ...
    await expect(templateLink).toHaveAttribute('href', 'https://github.com/srnarasim/dataprism-app-template');
  ...
  test('accessibility features work correctly', async ({ page }) => {
    ...
    const styles = await page.getByRole('heading', { name: 'DataPrism App Template' }).evaluate((el) => {
